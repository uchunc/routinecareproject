...
class _JournalViewerState extends State<JournalViewer> {
  ...
  void _loadEntries() {
    ...
    setState(() {
      ...
      imagePaths = List.generate(currentEntries.length, (index) => null); // 각 일지에 대해 이미지 경로를 null로 초기화
      controllers = List.generate(currentEntries.length, (index) => TextEditingController(text: currentEntries[index])); // 텍스트 컨트롤러 추가
    });
  }
